...
export default function HomePage() {
  ...
  const palette: PaletteItem[] = [
    ...
    { id: 5, type: "gate", name: "Rotation (θ)", op: "Rx", parameter: Math.PI / 4 } as Gate, // axis switchable in editor
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      ...
      <div
        className="h-screen w-screen p-4 grid grid-cols-[320px_1fr_360px] gap-4"
        ...
        <div className="bg-zinc-900/60 p-3 rounded border border-zinc-800/60 flex flex-col gap-3">
          ...
          {results && results.length > 0 && (
            <div className="text-xs text-zinc-300">
              {renderBV
                ? `x=${renderBV.x.toFixed(3)}, y=${renderBV.y.toFixed(3)}, z=${renderBV.z.toFixed(3)}`
                : "—"}
            </div>
          )}
        </div>

        {/* RIGHT: Palette + Editor (taller to avoid crowding) */}
        <div className="bg-zinc-900/60 p-3 rounded border border-zinc-800/60 flex flex-col gap-3">
